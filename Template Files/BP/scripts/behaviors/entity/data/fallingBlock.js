...
import { BlockInventoryComponent, ItemStack, StructureSaveMode, system, world } from "@minecraft/server";
...
class FallingBlock {
  static create(options) {
    ...
    if (options.custom) {
      ...
      const structureId = structure == null ? void 0 : structure.replaceAll("entity_id", entity.id);
      entity.setDynamicProperty("block", block);
      if (structureId) entity.setDynamicProperty("structure", structureId);
      try {
        entity.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 ${block}`);
      } catch (e) {
      }
      ...
      if (!block.isValid || block.isLiquid || block.isAir) return void 0;
      ...
      const inv = (_a = block.getComponent(BlockInventoryComponent.componentId)) == null ? void 0 : _a.container;
      const isComplex = inv !== void 0 || Object.entries(block.permutation.getAllStates()).length > 0;
      if (isComplex) {
        ...
        inv == null ? void 0 : inv.clearAll();
      }
      block.setType("air");
      try {
        entity.runCommand(`replaceitem entity @s slot.weapon.mainhand 0 ${typeId}`);
      } catch (e) {
      }
      entity.setDynamicProperty("block", typeId);
      if (rotating) entity.setProperty(`${namespace}:rotating`, rotating);
      if (isComplex) {
        entity.setDynamicProperty("structure", `${namespace}:falling_block.${entity.id}`);
      }
      return entity;
    } else return void 0;
  }
}
__publicField(FallingBlock, "behavior", {
  scriptEvents: {
    "turn_to_block": {
      callback: (entity) => {
        system.runTimeout(() => {
          ...
          const blockId = entity.getDynamicProperty("block");
          const location = { x: entity.location.x, y: entity.location.y + 0.2, z: entity.location.z };
          const structure = entity.getDynamicProperty("structure");
          entity.remove();
          if (!blockId) return;
          function dropBlock() {
            if (!blockId) return;
            try {
              spawnItem(new ItemStack(blockId, 1), dimension, location, true);
            } catch (e) {
            }
          }
          ...
          const complexBlockConditions = {
            "minecraft:snow_layer": (block2) => {
              return block2.permutation.getState("height") === 0;
            }
          };
